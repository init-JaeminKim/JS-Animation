...
function drawRare_amongUs() {
    var r_amongUs;
    clearCanvas_amongUs();
    ctx_amongUs.fillStyle = "rgba(255,255,255, 0.4)";
    ...
    for (var i = 0; i < rare_amongUs.length; i++) {
        ...
        ctx_amongUs.arc(rare_amongUs[i][0] += 0.5, rare_amongUs[i][1], 1.7, 0, Math.PI * 2, true);
    }
    ctx_amongUs.fill();
...
function drawFront_amongUs() {
    var r_amongUs;
    ctx_amongUs.fillStyle = "rgba(255,255,255, 0.8)";
    ctx_amongUs.beginPath();
    for (var i = 0; i < front_amongUs.length; i++) {
        r_amongUs = (Math.random() * 3) + 1;
        ctx_amongUs.moveTo(front_amongUs[i][0], front_amongUs[i][1]);
        ctx_amongUs.arc(front_amongUs[i][0] += 1, front_amongUs[i][1], 2.5, 0, Math.PI * 2, true);
    }
    ctx_amongUs.fill();

    ctx_amongUs.font = "12px Arial";
    ctx_amongUs.fillText("Jaemin was a developer", 38, 100);
}
